<!--
  ...
  A specialised text editor for HL7 messages with syntax highlighting and field navigation.
  ...
  2. Synchronise scroll positions between the two elements
  3. Make the textarea text transparent while keeping the caret visible

  This approach preserves all native textarea behaviours (selection, clipboard, IME support)
  ...
  - Validation highlighting for schema validation issues (error/warning/info)
  ...
  Validation Highlighting Integration:
  The editor accepts validationHighlights prop for schema validation feedback. Each highlight
  specifies a character range and severity (error/warning/info). The backend wraps these
  ranges in <span class="validation-{severity}"> tags, styled with underlines matching
  the severity color (red for errors, gold for warnings, blue for info).
  ...
  2. handleInput calls backend syntaxHighlight command with search/diff/validation highlights
  3. Highlighted HTML (including all highlight types) is injected into overlay div
...
<script lang="ts">
  ...
  let {
    ...
    validationHighlights?: ValidationMatch[];
  ...
  // or when search/diff/validation highlights change
  ...
  $effect(() => {
    ...
    const _validationHighlights = validationHighlights;

    if (editElement && highlightElement) {
      (editElement as HTMLTextAreaElement).value = _message ?? "";
      if (_message) {
        syntaxHighlight(_message, _searchMatches, _currentMatchIndex, _diffHighlights, _validationHighlights).then(
          (highlighted) => {
   ...
   *   - Prevents default tab behaviour (which would move focus to next element)
