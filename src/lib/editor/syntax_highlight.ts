 ...
 * 5. CSS rules apply colours to different element classes
 *
 * ## Why Overlay Instead of Rich Text Editor?
 *
 * Using a plain textarea with a highlighted HTML overlay preserves native text
 * editing behaviour (cursor, selection, undo/redo) while still providing visual
 * syntax highlighting. Rich text editors have accessibility and behavioural quirks.
 */
...
/**
 * Severity level for validation highlighting.
 */
export type ValidationSeverity = "error" | "warning" | "info";

/**
 * Represents a validation highlight range with its severity.
 */
 ...
 * @param validationMatches - Optional array of validation highlight ranges with severities
...
export async function syntaxHighlight(
  ...
  validationMatches?: ValidationMatch[],
): Promise<string> {
  return invoke("syntax_highlight", {
    ...
    validationMatches: validationMatches ?? null,
  });
}
