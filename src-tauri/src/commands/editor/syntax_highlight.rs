...
/// Severity level for validation highlights.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Deserialize)]
#[serde(rename_all = "lowercase")]
...
impl ValidationSeverity {
    ...
    fn class(&self) -> &'static str {
        match self {
            ValidationSeverity::Error => "validation-error",
            ValidationSeverity::Warning => "validation-warning",
            ValidationSeverity::Info => "validation-info",
        }
    }
}

/// A range representing a validation highlight with its severity.
#[derive(Debug, Clone, serde::Deserialize)]
pub struct ValidationMatch {
    /// Start position of the validation issue (byte offset)
    pub start: usize,
    /// End position of the validation issue (byte offset, exclusive)
    pub end: usize,
...
/// * `validation_matches` - Optional list of validation highlight ranges with their severities
///
/// # Returns
...
pub fn syntax_highlight(
    ...
    validation_matches: Option<Vec<ValidationMatch>>,
) -> String {
...
/// * `validation_matches` - Optional slice of validation highlight ranges with their severities
///
/// # Returns
...
fn do_syntax_highlight(
    ...
    validation_matches: Option<&[ValidationMatch]>,
) -> String {
...
/// Determines the validation highlight state for a given position.
///
/// Returns the validation severity if the position is within a validation range, or None if not.
fn get_validation_state(
    pos: usize,
    validation_matches: Option<&[ValidationMatch]>,
) -> Option<ValidationSeverity> {
    let matches = validation_matches?;

    for m in matches {
        if pos >= m.start && pos < m.end {
            return Some(m.severity);
        }
    }
    None
}
...
/// # Span Optimisation
///
/// The algorithm only opens/closes spans when the type changes. This minimises the
...
fn generate_html(
    ...
    for (i, c) in raw_message.char_indices() {
        ...
        // Handle validation span transitions
        if validation_state != current_validation_state {
            // Close previous validation span if we were in one
            if current_validation_state.is_some() {
                highlighted.push_str("</span>");
            }

            // If we're transitioning validation state, we need to close and reopen the syntax span
            if current_type.is_some() {
                highlighted.push_str("</span>");
                current_type = None;
            }
        ...
        // Open new validation span if entering a validation issue
        if validation_state != current_validation_state {
            if let Some(severity) = validation_state {
                highlighted.push_str(&format!(r#"<span class="{}">"#, severity.class()));
            }
        }

        current_match_state = match_state;
        current_diff_state = diff_state;
        current_validation_state = validation_state;
    ...
    // Close any open validation span
    if current_validation_state.is_some() {
        highlighted.push_str("</span>");
    }
