    ...
    for (name, occurrence) in sorted_keys {
        ...
        match (left_seg, right_seg) {
            (Some(ls), Some(rs)) => {
                ...
                summary.total_field_changes += fields
                    .iter()
...
fn build_segment_map<'a>(message: &'a Message<'a>) -> BTreeMap<(String, usize), &'a Segment<'a>> {
...
fn compare_segment_fields<'a>(
    ...
    for field_idx in 0..max_field {
        ...
        // field number is 1-indexed (field_idx 0 is field 1)
                ...
                if !value.is_empty() {
                    *has_changes = true;
                    diffs.push(FieldDiff {
                        path: format!(
                            "{}.{}{}.{}",
                            segment_name, field_num, repeat_suffix, comp_num
                        ),
                        description: None,
                        diff_type: DiffType::Removed,
                        left_value: Some(value),
