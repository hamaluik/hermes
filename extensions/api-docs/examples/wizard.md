# Wizard Extension Example

This advanced example demonstrates a complete wizard extension with:

- Web-based user interface served via HTTP
- Database lookup simulation with search results selection
- Full patient data population
- Schema overrides for custom fields
- Proper resource cleanup
- Window management via `ui/closeWindow`

## What It Does

1. Adds a "Patient Lookup" button to the toolbar
2. When clicked, opens a modal window with a search form
3. User enters a patient MRN or last name and clicks Search
4. Extension searches and shows matching patients in a list
5. User selects a patient and clicks Apply
6. Patient data is populated into the HL7 message
7. Window closes via `ui/closeWindow` API
...
Save this as `wizard_extension.py`:
...
### 1. Window Management with ui/closeWindow

The extension tracks the window ID and closes it via the API:
...
**Important:** JavaScript's `window.close()` does not work in Hermes windows. Extensions
must use the `ui/closeWindow` API to close windows programmatically.

### 2. Search Results UI

The wizard has a 3-state flow:

1. **Search View** - User enters MRN or last name
2. **Results View** - Shows matching patients in a selectable list
3. **Loading View** - Shown during API calls
...
```python
# extension sends request to Hermes
response = send_request("editor/patchMessage", {"patches": [...]})

# but also handles incoming requests while waiting
if "result" in msg or "error" in msg:
    handle_response(msg)
```
...
1. Make executable: `chmod +x wizard_extension.py`
2. Add to Hermes settings
3. Restart Hermes
4. Open a message with a PID segment
5. Click the search icon in the toolbar
6. Enter a search term: `DOE`, `SMITH`, `123`, or `87654321`
7. Click Search
8. Select a patient from the results list
9. Click Apply
10. Verify PID fields are populated and window closes

## Extending This Example

### Real Database Integration

Replace the simulated database with actual queries:

```python
import sqlite3

def lookup_patient(mrn):
    conn = sqlite3.connect("/path/to/patients.db")
    cursor = conn.cursor()
    cursor.execute("""
        SELECT mrn, last_name, first_name, dob, sex
...
Extend patches to populate more segments:

```python
patches.extend([
    {"path": "NK1.2.1", "value": emergency_contact["lastName"]},
    {"path": "NK1.2.2", "value": emergency_contact["firstName"]},
    {"path": "NK1.5.1", "value": emergency_contact["phone"]},
    {"path": "PV1.2", "value": "O"},  # outpatient
    {"path": "PV1.44", "value": admit_datetime},
])
```

## Related Documentation

- [UI Messages](../messages/ui.md) - Window API details
- [Editor Messages](../messages/editor.md) - Patching messages
